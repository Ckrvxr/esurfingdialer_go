...
func TestSM4StandardVector(t *testing.T) {
	...
	if toHex(out) != toHex(expected) {
		t.Fatalf("encrypt mismatch:\ngot:  %s\nexp:  %s", toHex(out), toHex(expected))
	}

	dec := make([]byte, 16)
	block.Decrypt(dec, out)
	if toHex(dec) != toHex(plain) {
		t.Fatalf("decrypt mismatch:\ngot:  %s\nexp:  %s", toHex(dec), toHex(plain))
	}
}
...
func TestSM4RoundTrip(t *testing.T) {
	...
	unpadded, err := Pkcs7UnPadding(dec)
	if err != nil {
		t.Fatal(err)
	}
